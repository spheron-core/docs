...
Make sure to read the output carefully and save your mnemonic phrase and wallet secret in a secure location. 

Once created, the wallet will be saved locally at `~/.spheron`. You can access the wallet-secret of the current wallet in `~/.spheron/config.json`. 
...
```sh
sphnctl wallet import --name [Wallet Name] --private-key [Private Key] --wallet-secret [Wallet Secret]
```

**Example:**

```sh
sphnctl wallet import --name wallet --private-key xxxxxxxxxx --wallet-secret xxxxxxxxxx
```
...
If you need to switch to a different wallet, use the command below, providing the name and the wallet-secret of the new wallet:

```sh
sphnctl wallet use --name [Wallet Name] --wallet-secret [Wallet Secret]
```

**Example:**

```sh
sphnctl wallet use --name wallet --wallet-secret xxxxxxxxxx
```
...
On-chain summary:
  State: Matched
  Price/hour: 0.005987447823663
  Started: 2025-07-27T20:57:31Z
  Gateway: 0xcbe31c8a8E1e1573A5D0cC40872e3D43D22e4D0f
  Fizz: 0x0fde61c805a8c0cbb73cbd824ac8ab7cd47bbf6c
Running services:
  ollama-test
    URIs: [http://67.220.70.94:20000]
    Ports:
      - 8080 → 20000 | https://dev-gateway-v2-bdpg-el.sphn.link
